...
// 추가: Azure SDK 오류 형태를 느슨하게 표현한 보조 타입 & 가드
type AzureError = Error & {
  code?: string;
  statusCode?: number;
  requestId?: string;
  details?: unknown;
...
function isAzureError(e: unknown): e is AzureError {
...
function getBlobService() {
  ...
  if (/yourAccount/i.test(raw) || /YOUR_KEY/i.test(raw)) {
    throw new Error(
      "AZURE_STORAGE_CONNECTION_STRING placeholder (yourAccount / YOUR_KEY) 를 실제 값으로 변경하세요"
    );
  }
  ...
  if (!hasAccountKey && (hasBlobEndpoint || hasSas)) {
    ...
    if (!sasToken)
      throw new Error(
        "Invalid SAS Connection String: SAS 토큰을 찾을 수 없습니다"
      );
...
export async function POST(req: NextRequest) {
  try {
    ...
    const isSasService =
      !/AccountKey=/i.test(process.env.AZURE_STORAGE_CONNECTION_STRING || "") &&
      /sv=/.test(service.url);
    if (!isSasService) {
      try {
        ...
        if (isAzureError(err)) {
          console.warn(
            "[upload API] container create warning",
            err.code,
            err.message
          );
        } else {
          console.warn("[upload API] container create warning", err);
        }
      }
    }
    ...
    const err = isAzureError(e) ? e : new Error("Unknown error");

    const diag: Record<string, unknown> = {
      message: err.message,
      code: (isAzureError(e) && e.code) ?? undefined,
      statusCode: (isAzureError(e) && e.statusCode) ?? undefined,
      requestId:
        (isAzureError(e) &&
          (e.requestId ?? e.response?.headers?.get?.("x-ms-request-id"))) ??
        undefined,
      details: (isAzureError(e) && e.details) ?? undefined,
    };
    ...
    let userMsg = (diag.message as string) || "upload 실패";
    if (diag.code === "OutOfRangeInput") {
      userMsg =
        "Azure: 요청 입력값이 허용 범위를 벗어났습니다 (파일 크기 / 이름 / 헤더 값 확인).";
    } else if (typeof userMsg === "string" && /placeholder/i.test(userMsg)) {
      userMsg =
        "환경변수에 예시 값이 그대로 남아 있습니다. 실제 스토리지 계정 연결 문자열을 넣어주세요.";
    } else if (diag.code === "AuthenticationFailed") {
      userMsg =
        "Azure 인증 실패: 연결 문자열(AccountName/Key) 혹은 시스템 시계를 확인하세요.";
    }
